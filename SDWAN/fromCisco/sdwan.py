#! /home/cisco/Git/devnet_env/bin/python
"""
...
# if vmanage_host is None or vmanage_port is None or vmanage_username is None :
...
def attached_devices(template):
    ...
    headers = ["Host Name", "Device IP", "Host ID", "Host Type"]
    table = list()

    for item in items:
        # tr = [item['host-name'], item['deviceIP'], item['site-id'], item['uuid'], item['personality']]
        tr = [item['host-name'], item['deviceIP'], item['uuid'], item['personality']]
        table.append(tr)
    try:
        click.echo(tabulate.tabulate(table, headers, tablefmt="fancy_grid"))
    except UnicodeEncodeError:
